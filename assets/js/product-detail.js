...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Hàm helper để chuyển chuỗi thành dạng slug (vd: "Bến Nghé" -> "ben-nghe")
    const toSlug = (str) => {
        if (!str) return '';
    ...
    function initPage() {
        ...
        if (!productId || !allData[productId]) {
            ...
            document.querySelector('.product-detail-layout').innerHTML = '<div style="padding: 40px; text-align: center;"><h1>Lỗi: Không tìm thấy sản phẩm</h1><p>Sản phẩm bạn tìm kiếm không tồn tại hoặc đã bị xóa. Vui lòng quay lại trang danh sách và thử lại.</p></div>';
            return;
        }
    ...
    // Thay thế toàn bộ hàm populatePageWithProductData cũ bằng hàm này
function populatePageWithProductData(product, isRental) {
    ...
    if (breadcrumbContainer) {
        ...
        breadcrumbParts.push(`<span class="breadcrumb-item-no-link">${propertyTypeText}</span>`);
        if (product.productCategory) {
            breadcrumbParts.push(`<span class="breadcrumb-item-no-link">${product.productCategory}</span>`);
        }
        if (product.city && citySlug && categorySlug) {
            const cityLink = `${listingPage}?loaihinh=${categorySlug}&thanhpho=${citySlug}`;
            breadcrumbParts.push(`<a href="${cityLink}">${product.city}</a>`);
        }
        if (product.ward && wardSlug && citySlug && categorySlug) {
            const wardLink = `${listingPage}?loaihinh=${categorySlug}&thanhpho=${citySlug}&phuong=${wardSlug}`;
            breadcrumbParts.push(`<a href="${wardLink}">${product.ward}</a>`);
        }
        if (product.street) {
             breadcrumbParts.push(`<span class="breadcrumb-item-no-link">${product.street}</span>`);
        }
    ...
    if (addressLine) {
        addressLine.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="18" height="18"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg> ${address}`;
    }

    const priceValueEl = document.querySelector('.price-value');
    const priceUnitEl = document.querySelector('.info-unit-price');
    if (isRental) {
    ...
    if (tbody) {
        ...
        if (isRental) {
            detailMap = [
                { label: "Nội thất", value: product.furniture }, { label: "Số phòng ngủ", value: product.bedrooms }, { label: "Số toilet", value: product.wc }, { label: "Tầng", value: product.floor }, { label: "View", value: product.view }, { label: "Hướng cửa", value: product.doorDirection }, { label: "Hướng ban công", value: product.balconyDirection || '-' }, { label: "Giá điện", value: product.electricityPrice || "Giá nhà nước" }, { label: "Giá nước", value: product.waterPrice || "Giá nhà nước" }, { label: "Internet", value: product.internetPrice || "Theo nhà mạng" }, { label: "Thông tin khác", value: product.otherInfo }, { label: "Loại sản phẩm", value: product.propertyType }, { label: "Loại hình", value: product.productCategory }, { label: "Thời hạn thuê", value: product.leaseTerm }, { label: "Mã tin", value: product.id }, { label: "Ngày cập nhật", value: new Date(product.publishedAt).toLocaleDateString('vi-VN') }
            ];
        } else {
            detailMap = [
                { label: "Pháp lý", value: product.legal }, { label: "Nội thất", value: product.furniture }, { label: "Số phòng ngủ", value: product.bedrooms }, { label: "Số toilet", value: product.wc }, { label: "Tầng", value: product.floor }, { label: "View", value: product.view }, { label: "Hướng cửa", value: product.doorDirection }, { label: "Hướng ban công", value: product.balconyDirection || '-' }, { label: "Thông tin khác", value: product.otherInfo }, { label: "Loại hình", value: product.productCategory }, { label: "Nhu cầu", value: product.propertyType }, { label: "Mã tin", value: product.id }, { label: "Ngày cập nhật", value: new Date(product.publishedAt).toLocaleDateString('vi-VN') }
            ];
        }
    ...
    const similarListingsLinkEl = document.getElementById('similar-listings-cta-link');
    if (similarWardNameEl && similarListingsLinkEl) {
        similarWardNameEl.textContent = `${product.ward}, ${product.city}`;
        
        // [CẬP NHẬT] Logic tạo link mới
        const targetPage = isRental ? 'listing-thue.html' : 'listing-ban.html';
        const categorySlug = toSlug(product.productCategory);
        const citySlug = toSlug(product.city);
        const wardSlug = toSlug(product.ward);

        // Xây dựng URL hoàn chỉnh với đầy đủ tham số
        similarListingsLinkEl.href = `${targetPage}?loaihinh=${categorySlug}&thanhpho=${citySlug}&phuong=${wardSlug}`;
    }
    ...
    if (projectLinkCard && product.project) {
        ...
        projectLinkCard.querySelector('.project-thumbnail').src = `assets/images/${product.projectImage}`;
    ...
    if (mapFrame) mapFrame.src = `https://maps.google.com/maps?q=${encodeURIComponent(address)}&output=embed`;
    ...
    contactInfoElements.forEach(config => {
        ...
        if (container) {
            ...
            const callBtn = container.querySelector(config.call); if(callBtn) callBtn.href = `tel:${product.brokerPhone}`;
            const zaloBtn = container.querySelector(config.zalo); if(zaloBtn) zaloBtn.href = `https://zalo.me/${product.brokerPhone.replace(/^0/, '')}`;
        }
    });
    ...
    function initHeroSlider(product) {
        ...
        let currentIndex = 0, isPaused = false, slideInterval, touchStartX = 0;
        const dots = [];
        dotsContainer.innerHTML = '';
        const dotsToShow = Math.min(slides.length, 3);
        ...
        const nextSlide = () => goToSlide(currentIndex + 1), prevSlide = () => goToSlide(currentIndex - 1), startInterval = () => { slideInterval = setInterval(nextSlide, 8000); }, resetInterval = () => { clearInterval(slideInterval); startInterval(); };
        playPauseBtn.addEventListener('click', () => { isPaused = !isPaused; track.classList.toggle('paused', isPaused); playPauseBtn.classList.toggle('paused', isPaused); isPaused ? clearInterval(slideInterval) : nextSlide(); });
        slider.addEventListener('touchstart', e => { touchStartX = e.touches[0].clientX; clearInterval(slideInterval); });
        slider.addEventListener('touchend', e => {
            ...
            } else { if (!isPaused) { resetInterval(); } }
            touchStartX = 0;
        });
        goToSlide(0);
    }
    ...
    if (typeof window.initFavoritesDrawer === 'function') { 
        window.initFavoritesDrawer(); 
    } else { 
        // This is expected if listing.js is not loaded on this page
    }
